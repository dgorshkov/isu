...
`
	...
	for _, want := range []string{
		...
		"<thead><tr>\n<th>head</th>\n<th>other</th>\n</tr></thead>",
		"<td>a</td>", "<hr>",
		`<div class="scroller sketch" tabindex="0" role="group" aria-label="example, not run">` +
...
func TestOnlyABlockThatRanLooksLikeOne(t *testing.T) {
	...
	require.Contains(t, got.HTML,
		`<div class="scroller ran" tabindex="0" role="group" aria-label="terminal output">`,
		"a console fence is executed by the build, so it keeps the terminal ground")
	require.Contains(t, got.HTML,
		`<div class="scroller sketch" tabindex="0" role="group" aria-label="example, not run">`,
		"every other fence is illustrative and must be visibly not a transcript — and must "+
			"say so to a reader who cannot see either ground")
}
